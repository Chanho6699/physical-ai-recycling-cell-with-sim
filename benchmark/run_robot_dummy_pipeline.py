...
LOG_PATH = PROJECT_ROOT / "results" / "logs" / "robot_dummy_pipeline_log.jsonl"
...
def run_pipeline(user_command: str) -> None:
    ...
    print("\n=== Apply to Dummy Robot Simulator ===")
    simulator = DummyRobotSimulator()
    robot_state_before = simulator.as_dict()
    simulator.apply_command(robot_command)
    robot_state_after = simulator.as_dict()

    print(f"Robot state before: {robot_state_before}")
    print(f"Robot state after:  {robot_state_after}")
