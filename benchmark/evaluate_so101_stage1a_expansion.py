"""Stage 1A expansion-validation / held-out expansion-test closed-loop
evaluator (see this task's chat report, "Stage 1A object XY 표적 보강
데이터셋 설계"/"...구현"). Evaluates a candidate checkpoint against the
NEW XY positions collected for Stage 1A (from
`datasets/so101_bin_stage1a_xy_70/stage1a_position_manifest.jsonl`),
using the exact same closed-loop machinery as the official
retention/baseline evaluations and the prior XY-extrapolation eval --
this file adds ONLY the "read positions from a manifest instead of a
fixed procedural list" plumbing; it reuses
benchmark.so101_smolvla_rollout's own load_policy_and_processors()/
run_one_rollout() (object_position_override, policy_noise_seed) and
robot_sim.so101_pybullet_backend.DEFAULT_OBJECT_POSITION unchanged.

Position ordering (needed for the derived policy-noise-seed formula
`base_seed + position_index*1000 + repeat_id`) is fixed and
deterministic: manifest rows for the requested split, sorted by
episode_index ascending -- so position_index is stable across
different checkpoint evaluations (validation) and stable independent
of validation once test positions get their own base seed (test).
...
Run:
  .venv-vla/bin/python -m benchmark.evaluate_so101_stage1a_expansion \\
    ...
    --policy-noise-base-seed 300000 --output-path results/.../expansion_validation.json
"""
...
DEFAULT_MANIFEST_PATH = PROJECT_ROOT / "datasets" / "so101_bin_stage1a_xy_70" / "stage1a_position_manifest.jsonl"
...
def derive_expansion_policy_noise_seed(base_seed: int, position_index: int, repeat_id: int) -> int:
...
def main() -> None:
    ...
    print(f"Loaded {len(positions)} '{args.split}' positions from {manifest_path}")
    ...
    for position_index, pos in enumerate(positions):
        ...
        for repeat_id in range(args.policy_noise_repeats):
            derived_seed = derive_expansion_policy_noise_seed(args.policy_noise_base_seed, position_index, repeat_id)
