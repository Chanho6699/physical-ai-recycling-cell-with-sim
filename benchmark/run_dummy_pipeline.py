...
LOG_PATH = PROJECT_ROOT / "results" / "logs" / "dummy_pipeline_log.jsonl"
...
def run_pipeline(instruction: str, image_path: Optional[str] = None) -> None:
    check_server_health()

    print("\n=== Request Action ===")
    action_response = request_action(instruction=instruction, image_path=image_path)

    action = action_response["action"]
    print(f"Instruction: {action_response['instruction']}")
    ...
    log_record = {
        ...
        "robot_command": robot_command.__dict__,
    }
