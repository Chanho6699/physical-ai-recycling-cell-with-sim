...
| Policy | `PolicyBackend` = `BasePolicy` (`policy/policy_backend.py`, `policy/base_policy.py`) | `DummyOpenVLAPolicy` (`--policy-backend local-dummy`), `FastAPIVLAPolicyClient` (`--policy-backend fastapi-dummy`, talks to `openvla_server_dummy/dummy_server.py`), `RealVLAPolicyClient` (`--policy-backend real-vla`, talks to `openvla_server_dummy/real_vla_compatible_server.py` locally, or to a Colab-hosted `openvla_server_real/colab_vla_server.py` over an ngrok/cloudflared tunnel -- see [docs/colab_vla_server_spike.md](colab_vla_server_spike.md)) |
...
| `real-vla-compatible-server`'s `DummyOpenVLAPolicy` internals (or the Colab spike's `mock-action`/`openvla-dryrun`) | A real OpenVLA (or other VLA) inference server behind the same `/predict`/`/health`/`/reset` contract and `configs/real_vla_backend_config.json` schema `RealVLAPolicyClient` already speaks -- only the server's internals need to change, not the client or the control loop. The Colab spike ([docs/colab_vla_server_spike.md](colab_vla_server_spike.md)) is a temporary test rig for this, not the hardware replacement itself -- a real deployment needs a persistent GPU server, not a notebook + tunnel |
...
- The Colab VLA Server Spike ([docs/colab_vla_server_spike.md](colab_vla_server_spike.md)) proves the local-machine-has-no-GPU problem is solvable (borrow one from Colab over a tunnel), but a real `openvla-dryrun` model load was never actually exercised end-to-end in this repo's own dev environment (no CUDA GPU here either) -- only its graceful-failure path (`model_status=not_loaded`) was verified.
